...
class TimeChannel(commands.Cog, TCLoop, metaclass=CompositeMetaClass):
    ...
    async def create(self, ctx: commands.Context, *, string: str):
        ...
        **For example**, running `[p]tcset short new york` gives a short identifier of `fv`.
        ...
        You can also use `{shortid-hr}` to return the hours untill 00:00, eg `{qw-hr}`
        ...
        **More Examples:**
        ...
        try:
            ...
            await ctx.send(
                ...
                f"`{p}tcset create New York: {{fv}}`, `{p}tcset create New York: {{fv-24h}}`, "
                f"or {p}tcset create New York: {{fv-hr}} hours till midnight"
            )
            return
